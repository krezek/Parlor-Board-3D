	...
	std::vector<RenderItem*>& fixedRenderItems)
{
	...
	auto bNewGameRitem = std::make_unique<RenderItem>();
	_newButton = bNewGameRitem.get();
	XMStoreFloat4x4(&bNewGameRitem->World, DirectX::XMMatrixScaling(0.1f, 0.02f, 0.1f)*
		DirectX::XMMatrixTranslation(0.45f, 0.35f, 0.0f));
	bNewGameRitem->ObjCBIndex = g_ObjCBIndex++;
	bNewGameRitem->Geo = geometries["fixedGeo"].get();
	bNewGameRitem->Mat = materials["new_game0"].get();
	bNewGameRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	bNewGameRitem->IndexCount = bNewGameRitem->Geo->DrawArgs["button"].IndexCount;
	bNewGameRitem->StartIndexLocation = bNewGameRitem->Geo->DrawArgs["button"].StartIndexLocation;
	bNewGameRitem->BaseVertexLocation = bNewGameRitem->Geo->DrawArgs["button"].BaseVertexLocation;
	bNewGameRitem->Bounds = bNewGameRitem->Geo->DrawArgs["button"].Bounds;

	fixedRenderItems.push_back(bNewGameRitem.get());
	allRitems.push_back(std::move(bNewGameRitem));
}
