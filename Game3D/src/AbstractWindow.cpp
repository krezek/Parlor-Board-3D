...
LRESULT AbstractWindow::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PROCESS_P1_MSG:
		OnProcessP1Msg(wParam, lParam);
		return 0;

	case WM_PROCESS_P2_MSG:
		OnProcessP2Msg(wParam, lParam);
		return 0;
...
LRESULT AbstractWindow::OnCreate()
{
	return 0;
}
...
void AbstractWindow::Create11On12Objects()
{
	...
	{
		ThrowIfFailed(_d2dDeviceContext->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::WhiteSmoke), &_textBrush));
...
void AbstractWindow::RenderUI()
{
	...
	D2D1_RECT_F textRect = D2D1::RectF(0, 0, rtSize.width / 3, rtSize.height / 3);
